...
function Accordion({num}) {
  ...
  return (
    <details open={Open} className='text-white px-[34px] py-[50px] block border-t border-x border-white/5 ' onClick={()=>setOpen(!Open)}>
        <summary className='list-none w-full flex justify-between items-center' onClick={()=>setOpen(!Open)}>
            <div className={(Open? "text-[#9eff00] ": "text-white " )+ "flex items-center gap-2"}>
                <span className='p-4 py-2 rounded-xl border border-[#2e2e2e] text-[24px] text-center font-semibold bg-[#242424]'>
                    {num}
                </span>
                <p className='text-[18px]'>What services does SquareUp provide?</p> 
            </div>
